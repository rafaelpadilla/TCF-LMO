...
from VDAO_folds.lib.hdf5dataset_v2 import HDF5Dataset
from VDAO_folds.lib.utils import target_objects as all_objects
...
if 'notesmt' in hostname:
    ...
    base_dir_save_torch = '/media/storage/VDAO/amostras_frames/geo_align_lmdb/'
    DIR_TO_SAVE_TRAIN_LMDB = os.path.join(base_dir_save_torch, 'geo_align_train_lmdb')
    DIR_TO_SAVE_VAL_TEST_LMDB = os.path.join(base_dir_save_torch, 'geo_align_val_test_lmdb')
    ...
    DIR_TO_SAVE_TRAIN_TORCH_FILES = os.path.join(base_dir_save_torch, 'geo_align_train_torch')
    DIR_TO_SAVE_VAL_TEST_TORCH_FILES = os.path.join(base_dir_save_torch, 'geo_align_val_test_torch')
    base_dir_save_torch = '/nfs/proc/rafael.padilla/geo_align_lmdb/'
    DIR_TO_SAVE_TRAIN_LMDB = os.path.join(base_dir_save_torch, 'geo_align_train_lmdb')
    DIR_TO_SAVE_VAL_TEST_LMDB = os.path.join(base_dir_save_torch, 'geo_align_val_test_lmdb')
else:
...
def get_file_paths_geo_aligned_hdf5_files(type_dataset, objects=None):
    if type_dataset in ['validation', 'test']:
        dir_to_read = DIR_FRAMES_VAL_TEST
    elif type_dataset == 'train':
        dir_to_read = DIR_FRAMES_TRAIN
    ret = []
...
def get_keys_lmdb(fold_number, type_dataset='train'):
    assert type_dataset in [
        'train', 'validation', 'test'
    ], 'Argument type_dataset must be either \'train\', \'validation\' or \'test\''

    lmdb_path = DIR_TO_SAVE_TRAIN_LMDB if type_dataset == 'train' else DIR_TO_SAVE_VAL_TEST_LMDB
