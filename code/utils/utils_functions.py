...
VDAO_resolution = {'height': 720, 'width': 1280}
...
def get_multiplication_mask(gt, no_border_output=False, to_device=None):
    if to_device is None:
        to_device = torch.device('cpu')
    if no_border_output:
        height, width = resnet_output_resolution_no_border[
            'height'], resnet_output_resolution_no_border['width']
    else:
        height, width = resnet_output_resolution['height'], resnet_output_resolution['width']
    # get amount of frames (batch)
    batch = len(gt['bounding_boxes'])
    # dx = width / VDAO_resolution['width'] # -> ONLY USED WITH TEMPORAL ALIGNMENT
    # dy = height / VDAO_resolution['height'] # -> ONLY USED WITH TEMPORAL ALIGNMENT
    dx = width / gt['shape'][-1]
    dy = height / gt['shape'][-2]
    # Creat N masks with 1's with the same resolution of the output of the network (n=batches)
    masks_ones = torch.zeros((batch, 1, height, width)).to(to_device)
...
def calculate_norm_mcc(output, gt, device, threshold=0.5):
    ...
    multiplication_mask = get_multiplication_mask(gt, no_border_output=True, to_device=device)
...
def calculate_best_window_temporal_consistency(output, gt, device, threshold=0.5):
    multiplication_mask = get_multiplication_mask(gt, no_border_output=True, to_device=device)
