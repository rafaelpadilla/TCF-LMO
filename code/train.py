...
@click.option('--name_experiment', default=None, help='name of the experiment.', required=True)
@click.option('--seed',
              default=123,
              help='random seed to achieve achieve reproducible results.',
              type=click.INT)
@click.option("--init_params_file", type=click.Path(exists=True), required=True)
@click.option("--continue_from", type=click.Path(exists=True), required=False)
@click.option("--tb_params_file", type=click.Path(exists=True), required=True)
def main(fold, epochs, batch_size, net, name_experiment, seed, init_params_file, tb_params_file,
         device, perform_validation, run_once_without_training, continue_from):
    ...
    if continue_from is None and dir_exists:
        ...
        if create_folder:
            create_folder = input(
                f'REALLY? ARE YOU SURE? ALL CONTENT WILL BE ERASED! (y: yes /n: no) ') == 'y'
            if create_folder:
    ...
    def train(cycle, not_learning=False):
        ...
        for batch, (ref_frames, tar_frames, labels_classes, bbs) in enumerate(data_loader_train):
            ...
            if cycle['cycle_name'] == 'training DM':
                ...
                norm_mcc = utils_functions.calculate_norm_mcc(output=outputs, gt=gts, device=device)
        ...
        if cycle['cycle_name'] == 'training DM':
            ...
            mean_loss = d[model.temporal_consistency.voting_window] / len(losses)
        elif cycle['cycle_name'] == 'training MM':
            mean_loss = np.mean(losses)
        elif cycle['cycle_name'] == 'training CM':
            mean_loss = np.mean(losses)
