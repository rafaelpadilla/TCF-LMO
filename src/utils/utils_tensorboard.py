...
def create_image_with_results(images,
    ...
    elif background in COLORS.keys():
        text_area = np.ones((3, text_area_height, sample_image_width), dtype=np.uint8)
        for c in range(len(text_area)):
            text_area[c] = text_area[c] * COLORS[background][c]
    else:
        constant_values = 0
    ...
    for idx in range(total_images):
        ...
        if add_border:
            if background in COLORS.keys():
                r = np.pad(img2[:, :, 0], ((0, 5), (5, 5)),
                           mode='constant',
                           constant_values=COLORS[background][0])
                g = np.pad(img2[:, :, 1], ((0, 5), (5, 5)),
                           mode='constant',
                           constant_values=COLORS[background][1])
                b = np.pad(img2[:, :, 2], ((0, 5), (5, 5)),
                           mode='constant',
                           constant_values=COLORS[background][2])
