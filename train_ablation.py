...
def main(fold, epochs, batch_size, ablation, name_experiment, seed, init_params_file,
         tb_params_file, device, perform_validation, run_once_without_training, continue_from,
         alignment):
    ...
    if ablation == 'modification1':
        ...
        # Evaluate the contribution of the TCM module :. Removing TCM.
        train_cycles = [{
            'cycle_name': 'training DM',
            'loss_func': nn.MSELoss(),
            'batch_size': batch_size,
        ...
        # Evaluate the contribution of the MM module :. Removing MM.
        train_cycles = [
            {
                'cycle_name': 'training DM',
                'loss_func': nn.MSELoss(),
        ...
        # Evaluate the contribution of the DM module :. Remove all modules, except the DM.
        train_cycles = [{
            'cycle_name': 'training DM',
            'loss_func': nn.MSELoss(),
            'batch_size': batch_size,
    ...
    # Create network
    ablation_cycles = [cycle['cycle_name'] for cycle in train_cycles]
    model = model_DM_MM_TCM_CM(init_params, ablation_cycles, ablation, device).to(device)

    hooks_dict = utils_functions.register_hooks(model)

    inference_cycle['batch_size'] = model.temporal_consistency.voting_window if hasattr(
        model, 'temporal_consistency') else train_cycles[-1]['batch_size']
    ...
    if hasattr(model, 'dissimilarity_module'):
        ...
        param_list.append({'params': model.dissimilarity_module.combination_bias, 'lr': 2e-3})
    if hasattr(model, 'opening'):
        param_list.append({'params': model.opening.parameters(), 'lr': 1e-4})
        param_list.append({'params': model.closing.parameters(), 'lr': 13e-3})
    param_list.append({'params': model.classification_function.parameters(), 'lr': 1e-4})
    optimizer = optim.Adam(param_list, lr=train_params['lr'])
    ...
    ###################################################
    # Training dataset
    print_info('Datasets info:', log_path)
    ...
    load_mode = 'keyframe' if hasattr(model, 'temporal_consistency') else 'block'

    # # AQUIII TIRA ISSO DAQUI
    # dataset_train.keys_ds = dataset_train.keys_ds[0:100]
    ...
    def prepare_samples(cycle, ref_frames, tar_frames, labels_classes, bbs):
        ...
        if cycle['cycle_name'] == 'training DM':
            ...
            # if len(ref_frames.shape) == 5 and ref_frames.shape[1] == 1:
            if not hasattr(model, 'temporal_consistency'):
                ref_frames = ref_frames.squeeze(1)
                tar_frames = tar_frames.squeeze(1)
        ...
        for batch, (ref_frames, tar_frames, labels_classes, bbs) in enumerate(data_loader_train):
            ...
            if save_images_tb:
                frames_to_save['dissimilarity_output'].append(
                    (hooks_dict['hook_opening'].input[0][middle_id] * 255).to(torch.uint8).cpu())
            ...
            if cycle['cycle_name'] == 'training DM':
                ...
                if hasattr(model, 'temporal_consistency'):
                    ...
                    # gts_labels = torch.tensor(gts['labels'] * 1.).to(device)
                    gts_labels = (gts['labels'] * 1.).to(device)
                loss = cycle['loss_func'](outputs.squeeze(), gts_labels)
                losses.append(loss.item())
                if not not_learning:
                    loss.backward()
                    optimizer.step()
    ...
    def validate(cycle, quiet=True):
        ...
        for id_vid_val, dataset_val in enumerate(datasets_validation):
            ...
            data_loader_validate = DataLoader(dataset_val,
                                              **loader_params_val,
                                              batch_size=inference_cycle['batch_size'])
            # prepare model (freezing modules that are not trained)
            ...
            for batch, (ref_frames, tar_frames, labels_classes,
                ...
                if hasattr(model, 'temporal_consistency'):
                    ...
                    for sample_id in range(samples_batch):
                        ...
                        loss = cycle['loss_func'](outputs.squeeze(),
                                                  (data['class'] * 1.).to(device))
                        losses_vid.append(loss.item())

                        # compute the metrics
                        class_out = (outputs > .5).item()
                        metrics_vid['gt_labels'].append((data['class'].item() == 1) * 1)
                        metrics_vid['pred_labels'].append(class_out * 1)
...
if __name__ == "__main__":
    ...
    # run_once_without_training = True
    # ablation = 'modification4'
    # name_experiment = 'testeABC'
    # seed = 123
    ...
    #      run_once_without_training=run_once_without_training,
    #      ablation=ablation,
    #      name_experiment=name_experiment,
    #      seed=seed,
    #      init_params_file=init_params_file,
    #      continue_from=continue_from,
    #      tb_params_file=tb_params_file,
    #      alignment=alignment)
    main()
